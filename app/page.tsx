...
export default function Home() {
  ...
  async function handleAdd(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const data = await request<CreateTodoResponse>("/api/todos", {
        method: "POST",
        body: JSON.stringify({ title: trimmed } satisfies CreateTodoRequest),
      });
  ...
  async function handleToggle(todo: Todo) {
    ...
    setError(null);
    setTodos((prev) =>
      prev.map((t) => (t.id === todo.id ? { ...t, isCompleted } : t)),
    );
    try {
      const data = await request<UpdateTodoResponse>(`/api/todos/${todo.id}`, {
        method: "PATCH",
        body: JSON.stringify({ isCompleted } satisfies UpdateTodoRequest),
      });
                  ...
                  {todos.map((todo) => (
                    <li key={todo.id} className="flex items-center gap-3 py-3">
                      ...
                      <button
                        ...
                        className="shrink-0 rounded-lg px-2.5 py-1 text-sm text-zinc-400 transition hover:bg-red-500/10 hover:text-red-400"
                      >
                        削除
                      </button>
                    </li>
